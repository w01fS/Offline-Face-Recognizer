...
while True:
    if datetime.now().time().hour == 00 and datetime.now().time().second < 5: #change the number to present hour to see it working
    ...
    if datetime.now().time().hour == 21 and datetime.now().time().minute == 6: #change the number to present hour to see it working
        conn = sqlite3.connect(sqlite_file)
